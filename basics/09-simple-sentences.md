<div dir="ltr">

# Chapter 9Simple Sentences  

</div>
## باب ۹: آسان جملے

### 📚 Welcome to Phase 3: Sentences & Reading!

Congratulations on reaching the final phase! Now it's time to put everything together and **make complete sentences**!

**مدت / Duration:** 5-6 days of practice  
**نیا تصور / New Concept:** Sentence structure, word order  
**ہدف / Goal:** Read and write simple Urdu sentences

---

<div dir="ltr">

## 🎯 Urdu Sentence Structure### Basic Word Order **Subject + Object + Verb**

</div>

**Example:**
- میں (I) + پانی (water) + پیتا ہوں (drink)  
- **میں پانی پیتا ہوں۔** (I drink water)
<div dir="ltr">

This is DIFFERENT from English

</div>
- English: I + drink + water (S-V-O)
- Urdu: میں + پانی + پیتا ہوں (S-O-V)

---

<div dir="ltr">

## 📖 Pattern 1 Simple Present Tense### Format Subject + Verb (present form)

</div>

<div class="practice-box">

| Urdu | English |
|------|---------|
| **میں جاتا ہوں۔** | I go. |
| **تم آتے ہو۔** | You come. |
| **وہ کھاتا ہے۔** | He eats. |
| **ہم پڑھتے ہیں۔** | We read. |
| **وہ سوتے ہیں۔** | They sleep. |

</div>

---

<div dir="ltr">

## 📖 Pattern 2 "To Be" Sentences (ہے / ہیں)### Format Subject + Adjective/Noun + ہے/ہیں

</div>

<div class="practice-box">

| Urdu | English |
|------|---------|
| **یہ گھر ہے۔** | This is a house. |
| **وہ اچھا ہے۔** | He is good. |
| **بچے خوش ہیں۔** | Children are happy. |
| **ماں پیاری ہے۔** | Mother is lovely. |
| **آسمان نیلا ہے۔** | The sky is blue. |

</div>

**Rule:** 
- Singular → **ہے** (hai)
- Plural → **ہیں** (hain)

---

<div dir="ltr">

## 📖 Pattern 3 Negative Sentences (نہیں)### Format Subject + Object + **نہیں** + Verb

</div>

<div class="practice-box">

| Positive | Negative |
|----------|----------|
| **میں جاتا ہوں۔** | **میں نہیں جاتا۔** |
| (I go) | (I don't go) |
| **وہ کھاتا ہے۔** | **وہ نہیں کھاتا۔** |
| (He eats) | (He doesn't eat) |
| **بچے کھیلتے ہیں۔** | **بچے نہیں کھیلتے۔** |
| (Children play) | (Children don't play) |

</div>

**Tip:** Just add **نہیں** (nahin) before the verb!

---

<div dir="ltr">

## 📖 Pattern 4 Question Sentences### Method 1 Add **کیا** at the beginning

</div>

<div class="practice-box">

| Statement | Question |
|-----------|----------|
| **تم جاتے ہو۔** | **کیا تم جاتے ہو؟** |
| (You go) | (Do you go?) |
| **وہ اچھا ہے۔** | **کیا وہ اچھا ہے؟** |
| (He is good) | (Is he good?) |

### Method 2: Use Question Words<div class="practice-box">

</div>

| Question Word | Urdu | Example |
|---------------|------|---------|
| What | **کیا** | **تم کیا کرتے ہو؟** (What do you do?) |
| Who | **کون** | **یہ کون ہے؟** (Who is this?) |
| Where | **کہاں** | **وہ کہاں ہے؟** (Where is he?) |
| When | **کب** | **تم کب آؤگے؟** (When will you come?) |
| Why | **کیوں** | **تم کیوں روتے ہو؟** (Why do you cry?) |
| How | **کیسے** | **یہ کیسے ہوا؟** (How did this happen?) |

---

<div dir="ltr">

## 📖 Common Sentence Patterns### Pattern 1 Daily Actions

</div>

<div class="practice-box">

1. **میں صبح اٹھتا ہوں۔**  
   (I wake up in the morning)

2. **میں ناشتہ کرتا ہوں۔**  
   (I have breakfast)

3. **میں سکول جاتا ہوں۔**  
   (I go to school)

4. **میں کتاب پڑھتا ہوں۔**  
   (I read a book)

5. **میں رات کو سوتا ہوں۔**  
   (I sleep at night)

### Pattern 2: Describing Things<div class="practice-box">

</div>

1. **آسمان نیلا ہے۔**  
   (The sky is blue)

2. **پھول خوبصورت ہیں۔**  
   (Flowers are beautiful)

3. **کتاب موٹی ہے۔**  
   (The book is thick)

4. **پانی ٹھنڈا ہے۔**  
   (Water is cold)

5. **سورج گرم ہے۔**  
   (The sun is hot)

### Pattern 3: Family Sentences<div class="practice-box">

</div>

1. **میری ماں اچھی ہے۔**  
   (My mother is good)

2. **میرا باپ کام کرتا ہے۔**  
   (My father works)

3. **میری بہن سکول جاتی ہے۔**  
   (My sister goes to school)

4. **ہم سب خوش ہیں۔**  
   (We all are happy)

5. **ہمارا گھر بڑا ہے۔**  
   (Our house is big)

</div>

---

## 💪 Practice Exercises

<div dir="ltr">

### Exercise 1 Translate to Urdu<div class="practice-box">

</div>

1. I eat bread.  
   **Answer:** میں روٹی کھاتا ہوں۔

2. You drink water.  
   **Answer:** تم پانی پیتے ہو۔

3. He reads a book.  
   **Answer:** وہ کتاب پڑھتا ہے۔

4. We go to school.  
   **Answer:** ہم سکول جاتے ہیں۔

5. They are happy.  
   **Answer:** وہ خوش ہیں۔

### Exercise 2: Make Negative<div class="practice-box">

</div>

1. **میں جاتا ہوں۔** → **میں نہیں جاتا۔**
2. **وہ کھاتی ہے۔** → **وہ نہیں کھاتی۔**
3. **بچے کھیلتے ہیں۔** → **بچے نہیں کھیلتے۔**

### Exercise 3: Make Questions<div class="practice-box">

</div>

1. **تم جاتے ہو۔** → **کیا تم جاتے ہو؟**
2. **وہ اچھا ہے۔** → **کیا وہ اچھا ہے؟**
3. **بچے خوش ہیں۔** → **کیا بچے خوش ہیں؟**

### Exercise 4: Fill in the Blanks<div class="practice-box">

</div>

1. میں _____ پیتا ہوں۔ (I drink water)  
   **Answer:** پانی

2. وہ _____ جاتا ہے۔ (He goes to school)  
   **Answer:** سکول

3. ماں _____ ہے۔ (Mother is good)  
   **Answer:** اچھی

</div>

---

<div dir="ltr">

## 📚 Vocabulary Connecting Words<div class="practice-box">

</div>

| Urdu | English |
|------|---------|
| **اور** | and |
| **لیکن** / **مگر** | but |
| **یا** | or |
| **کیونکہ** | because |
| **اگر** | if |
| **تو** | then |
| **جب** | when |
| **پھر** | then/after |

### Using Connecting Words:1. **میں کھانا کھاتا ہوں اور پانی پیتا ہوں۔**  

</div>
   (I eat food and drink water)

2. **وہ جاتا ہے لیکن میں نہیں جاتا۔**  
   (He goes but I don't go)

3. **تم چائے یا کافی پیتے ہو؟**  
   (Do you drink tea or coffee?)

---

## 📅 5-Day Practice Schedule

<div dir="ltr">

### Day 1 Basic Sentences- Learn S-O-V structure

</div>
- Practice Pattern 1 (Simple Present)
- Write 10 simple sentences
- Exercise 1

<div dir="ltr">

### Day 2 "To Be" & Negatives- Practice Pattern 2 (ہے/ہیں)

</div>
- Learn negative formation
- Practice Exercise 2

<div dir="ltr">

### Day 3 Questions- Learn question formation (کیا)

</div>
- Practice question words
- Practice Exercise 3

<div dir="ltr">

### Day 4 Complex Sentences- Learn connecting words

</div>
- Combine simple sentences
- Practice Exercise 4

<div dir="ltr">

### Day 5 Master Practice- Write 20 sentences about your day

</div>
- Translate 10 English sentences to Urdu
- Read all example sentences
- Complete all exercises

---

## ✅ Learning Outcomes
<div dir="ltr">

By the end of this chapter, you should be able to

</div>
- ✅ Understand Urdu sentence structure (S-O-V)
- ✅ Form simple present tense sentences
- ✅ Use ہے/ہیں correctly
- ✅ Make negative sentences with نہیں
- ✅ Ask questions using کیا and question words
- ✅ Connect sentences with اور، لیکن، یا
- ✅ Write 20+ simple sentences independently

---

<div dir="ltr">

## 🎉 Excellent ProgressYou can now make complete Urdu sentences

</div>

**Next:** Chapter 10 - Expand your sentence skills with more tenses and complex structures!

**بہترین!** (Excellent!)
